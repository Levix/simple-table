...
export default function useSortable() {
  let sortableData = ref([] as Record<string, any>)

  /**
   * 记录点击的key的次数：0,1,2
   * 此处的key一定是 sring 类型，是外部自定义的 sortable：true 对应的字段
   */
  let keyClickMap = ref({} as Record<string, number>)
   ...
   */
  const handleSort = (originData: Record<string, any>[], key: string, isMultiSort: boolean = false, isLocalSort: boolean = false) => {
